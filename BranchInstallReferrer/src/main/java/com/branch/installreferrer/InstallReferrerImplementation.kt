...
class InstallReferrerImplementation : InstallReferrerInterface {
    ...
    override fun fetchInstallReferrerData(context: Context, callback: InstallReferrerFetchEvents) {
        scope.launch {
            try {
                fetchInstallReferrer(context, callback)
            } catch (e: Exception) {
                e.printStackTrace()
            }
        }
    ...
    private suspend fun fetchInstallReferrer(context: Context, callback: InstallReferrerFetchEvents?) {
        v("Begin fetchInstallReferrer")
        try {
            val latestReferrer = fetchLatestInstallReferrer(context)

            if (latestReferrer != null) {
                v("fetchInstallReferrer got result: $latestReferrer")

                AppStoreReferrer.processReferrerInfo(
                    context,
                    latestReferrer.installReferrer,
                    latestReferrer.referrerClickTimestampSeconds,
                    latestReferrer.installBeginTimestampSeconds,
                    latestReferrer.appStore,
                    latestReferrer.isClickThrough,
                    latestReferrer.installBeginTimestampServerSeconds,
                    latestReferrer.referrerClickTimestampServerSeconds
                )
            } else {
                v("fetchInstallReferrer got null result")
            }
