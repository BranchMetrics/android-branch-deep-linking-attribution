...
class ReferringUrlUtilityTests : BranchTest() {
    ...
    private fun openServerRequest(): ServerRequest {
        ...
        return ServerRequest.fromJSON(jsonObject, Branch.instance!!.applicationContext)
    }

    @Before
    fun initializeValues() {
        initBranchInstance()
        referringUrlUtility = ReferringUrlUtility(PrefHelper.getInstance(Branch.instance!!.applicationContext))
    }
    ...
    fun testCheckForAndMigrateOldGclid() {
        PrefHelper.getInstance(Branch.instance!!.applicationContext).setReferringUrlQueryParameters(null);
        val expected = JSONObject("""{"gclid": "12345", "is_deeplink_gclid": false}""")

        PrefHelper.getInstance(Branch.instance!!.applicationContext).referrerGclid = "12345"
        PrefHelper.getInstance(Branch.instance!!.applicationContext).referrerGclidValidForWindow = 2592000;

        val utility = ReferringUrlUtility(PrefHelper.getInstance(Branch.instance!!.applicationContext))
