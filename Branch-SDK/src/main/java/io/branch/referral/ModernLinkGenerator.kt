...
class ModernLinkGenerator(
    ...
    internal fun generateShortLinkSync(
        ...
        BranchLogger.t("MODERNIZATION_TRACE: ModernLinkGenerator.generateShortLinkSyncFromJava called")
        if (linkData == null) return if (defaultToLongUrl) longUrl else null
        
        // First try modern coroutine-based approach
        try {
            BranchLogger.t("MODERNIZATION_TRACE: Using Kotlin coroutines for link generation")
            return runBlocking {
                val result = generateShortLink(linkData, (timeout + 2000).toLong())
                val url = result.getOrNull()
                if (url != null) {
                    BranchLogger.t("MODERNIZATION_TRACE: Modern coroutine link generation succeeded")
                    url
                } else {
                    BranchLogger.t("MODERNIZATION_TRACE: Modern coroutine link generation returned null, using fallback")
                    if (defaultToLongUrl) longUrl else null
                }
            }
        } catch (e: Exception) {
            BranchLogger.t("MODERNIZATION_TRACE: Modern link generation failed, falling back to legacy: ${e.message}")
        }
        
        // Fallback to dedicated legacy utility class for maximum compatibility
        BranchLogger.t("MODERNIZATION_TRACE: Using BranchLegacyLinkGenerator fallback")
    ...
    internal fun generateShortLinkAsync(
        ...
        BranchLogger.t("MODERNIZATION_TRACE: ModernLinkGenerator.generateShortLinkAsyncFromJava called")
        scope.launch {
            try {
                if (linkData == null) {
                    withContext(Dispatchers.Main) {
