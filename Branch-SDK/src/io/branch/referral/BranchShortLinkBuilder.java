...
public class BranchShortLinkBuilder extends BranchUrlBuilder<BranchShortLinkBuilder> {
     ...
     * call; with a duration specified within which an app session should be matched to the link.</p>
     *
     * @param defaultToLongUrl A {@link Boolean} whose value should be false if a  long url is not intended to return on failure.
     *                         Setting this it false will return NULL for url on failure else a long url.
     * @return A {@link String} containing the resulting short URL. Null is returned in case of an error or if Branch is not initialised.
     */
    public String getShortUrl(boolean defaultToLongUrl) {
        return super.getUrl(defaultToLongUrl);
    }
     ...
     *                         Setting this it false will return NULL for url on failure else a long url.
     */
    public void generateShortUrl(Branch.BranchLinkCreateListener callback, Boolean defaultToLongUrl) {
        super.generateUrl(callback, defaultToLongUrl);
    }


    void generateShortUrlInternal(Branch.BranchLinkCreateListener callback, boolean isFromShareSheet) {
        super.generateUrlInternal(callback, isFromShareSheet, true);
    }
}
